---
last_mapped: 2026-04-07T03:00:00Z
total_files: 83
total_tokens: 135045
---
...
> Auto-generated by Cartographer. Last mapped: 2026-04-07
...
│   └── .env.example                 # Documents required env vars (EXA_API_KEY)
├── tests/
...
│   └── superpowers/               # Implementation plans + specs (historical reference)
├── research/                      # Pipeline run outputs (gitignored)
│   ├── runs/                      # Timestamped run directories
│   └── latest -> runs/...         # Symlink to most recent run
├── LICENSE                        # MIT
├── README.md                      # Dual-path quickstart: pip install vs full pipeline
├── portals.yml                    # Target company registry for ATS scanning (gitignored)
...
**80 tests** across 15 test files. Run with: `.venv/bin/pytest`
...
| 4 | `setup_env()` | Prompts for Exa API key; writes `.env`; skips if already exists |
